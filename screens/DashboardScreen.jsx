...
export default function DashboardScreen() {
  ...
  const userName = 'Ashwin'; // Example user name
  const balance = 12500;
  const income = 8000;
  const expenses = 3000;
        ...
        <View style={theme.navbar}>
          ...
          <View style={theme.navLinksRow}>
            <TouchableOpacity onPress={() => navigation.navigate('Home')}>
              <Text style={theme.navLink}>Home</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => navigation.navigate('Trackonomy')}>
              <Text style={theme.navLink}>Next Budget</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => navigation.navigate('Transactions')}>
              <Text style={theme.navLink}>Transactions</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => navigation.navigate('Settings')}>
              <Text style={theme.navLink}>Settings</Text>
            </TouchableOpacity>
          </View>
...
const lightStyles = StyleSheet.create({
  ...
  navLinksRow: { flexDirection: 'row', justifyContent: 'space-around', marginVertical: 8 },
  navLink: { fontSize: 16, color: '#555' },
  toggleButton: { position: 'absolute', top: 10, right: 16 },
  toggleIcon: { fontSize: 24, color: '#555' },
  greeting: { fontSize: 24, fontWeight: 'bold', marginTop: 20, marginBottom: 10, color: '#333' },
  balanceCard: { backgroundColor: '#007bff', borderRadius: 12, padding: 20, alignItems: 'center', marginBottom: 20 },
  ...
  investmentCard: { backgroundColor: '#e2e3e5', borderRadius: 12, padding: 20, alignItems: 'center', marginBottom: 20 },
  investmentAmount: { fontSize: 24, fontWeight: 'bold', marginTop: 8, color: '#333' },
  transactionItem: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#ccc' },
  transactionText: { fontSize: 16, color: '#333' },
  transactionAmount: { fontSize: 16, fontWeight: 'bold' },
  quickActions: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 20 },
  quickButton: { backgroundColor: '#007bff', padding: 14, borderRadius: 10, flex: 0.48, alignItems: 'center' },
